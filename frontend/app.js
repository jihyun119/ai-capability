...
function t3Screens() {
  const t3IntroCards = [
    ...
    ["리포트 결과", "7개 평가축을 기준으로 잘한 점, 보완할 점, 개선 포인트를 받아요."],
  ].map(([title, desc]) => `<article><strong>${title}</strong><span>${desc}</span></article>`).join("");

  const t3ScenarioCards = [
    {
      tags: ["의사결정", "PRD", "우선 순위"],
      title: "PM",
      desc: "3주 안에 만들 핵심 기능을 결정하고, 다음 회의에 바로 쓸 수 있는 PRD 초안을 만듭니다.",
    },
    {
      tags: ["리텐션", "CRM", "예산 배분"],
      title: "마케팅",
      desc: "재구매율 개선 캠페인을 기획하고, 제한된 예산과 채널 안에서 실행안을 설계합니다.",
    },
    {
      tags: ["문제정의", "지표분석", "경영진 보고"],
      title: "데이터 분석",
      desc: "성과 부진 원인을 분석하기 위한 지표와 분석 계획을 경영진 보고 형태로 정리합니다.",
    },
  ].map((item, index) => `
    <button class="t3-scenario-card" type="button" data-t3-scenario="${index}">
  ...
  return [
      ...
      <section class="t3-intro-layout">
        <div class="t3-intro-copy">
          ...
          <div class="t3-intro-characters">
            ...
            ${char("result", "t3-character t3-character-b")}
          </div>
        </div>
        <div class="t3-intro-cards">${t3IntroCards}</div>
      </section>
    ...
    screen(
      ...
      <section class="t3-scenario-layout">
        ...
        <section class="t3-scenario-list">${t3ScenarioCards}${button("다음", "t3-chat", "primary", "t3-scenario-next")}</section>
      </section>`,
    ...
    screen(
      ...
      <section class="t3-chat-layout">
        <aside class="t3-chat-brief">
          <h2>상황 설명</h2>
          <p>당신은 키키오 선물하기 서비스 개선 팀의 PM입니다. 다음 분기 3주 동안 만들 핵심 기능 하나를 정해야 합니다.</p>
          <p>개발자는 쿠폰함 알림 기능을, 디자이너는 위시리스트 공유 기능을, 데이터 분석가는 구매 후 추천 기능을 각각 1순위로 제안했습니다.</p>
          <p>기간은 3주뿐이라 셋 중 하나만 선택해야 합니다. AI에게 후보별 비교와 의사결정 프레임워크를 요청하고, 최종적으로 다음 회의에 바로 쓸 수 있는 PRD 초안을 만들어보세요.</p>
          <h2>미션 가이드</h2>
          <p>다음 내용을 중심으로 AI와 함께 의사결정을 진행해보세요.</p>
          <ul><li>비교 기준 설정과 후보안별 비교 평가</li><li>선택과 선정 근거</li><li>PRD 핵심 항목</li></ul>
        </aside>
    ...
    screen(
      "t3-loading",
      "T3-04 분석 로딩",
      `${header()}<section class="t3-loading-content"><h1>AI와 작업한 과정을<br />분석하는 중...</h1><img src="./Logo/Logo.v1.png" alt="" /></section>`,
    ...
    screen(
      ...
      <section class="t3-result-layout">
        <article class="t3-result-score">
          <strong class="t3-grade">실무 적응형</strong>
          <h1>74점</h1>
          <div class="t3-score-list">${scoreRows}</div>
        </article>
        <article class="t3-result-report">
          ...
          <div class="t3-detail-list">${detailRows}</div>
        </article>
      </section>
...
function showScreen(name) {
  ...
  if (name === "t3-loading") {
    ...
    showScreen.loadingTimer = window.setTimeout(() => showScreen("t3-result"), 1500);
  }
}
...
document.addEventListener("click", async (event) => {
  ...
  const t3Scenario = event.target.closest("[data-t3-scenario]");
  if (t3Scenario) {
    event.preventDefault();
    t3Scenario.parentElement.querySelectorAll("[data-t3-scenario]").forEach((buttonNode) => buttonNode.classList.remove("is-selected"));
    t3Scenario.classList.add("is-selected");
    return;
  }
